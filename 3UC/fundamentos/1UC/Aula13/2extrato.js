...
class Conta{ //essa variável é do tipo conta. Funçao dentro de classe é chamado método.
    ...
    depositar(valor){
        if(valor > 0){
            ...
            this.extrato.push("Depósito realizado com sucesso!")
        }
    }
    realizarSaque(valor){
        if(valor <= 0){
    ...
    transferencia(valor, conta){
        this.extrato.push('Início da transferência')
        this.realizarSaque(valor)
        conta.depositar(valor)
        this.extrato.push('Transferência realizada.')
    }
    gerarExtrato(){
...
conta1.transferencia(200, conta2) //agora a transferência foi da minha conta para a outra
